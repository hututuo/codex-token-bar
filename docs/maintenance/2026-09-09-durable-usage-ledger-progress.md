...
2. **覆盖旧发布和中间版本。** Swift schema 6/7/11/12 → 13，Rust schema 9/10/11/12 → 13；结构候选契约仍为 11，会计仍为 `codex-components-v1`，账本组件为 1，保护层为 schema 2 / writer 4。未知版本或 schema13 缺账本表时先拒绝，不自动清空重建。旧程序的未来版本判断会拒绝 schema13；这不承诺降级后继续精确增量。
