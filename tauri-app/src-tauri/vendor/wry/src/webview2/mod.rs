...
// WebView2 completion callbacks run on the creating STA thread. On Windows ARM64,
// webview2_com::wait_with_pump cannot deliver the callback for a second controller
// because its nested Win32 pump does not yield the apartment to COM. Keep the
// existing upstream path on x86/x64 and use the COM-sanctioned wait only where the
// deadlock is deterministic. See wry#1665 and the independent patch provenance in
// README.codex-token-bar.md.
#[cfg(target_arch = "aarch64")]
struct Arm64ComCompletion<T> {
  event: HANDLE,
  result: Rc<RefCell<Option<windows::core::Result<T>>>>,
}

#[cfg(target_arch = "aarch64")]
impl<T> Arm64ComCompletion<T> {
  fn new() -> windows::core::Result<Self> {
    Ok(Self {
      event: unsafe { CreateEventW(None, true, false, PCWSTR::null())? },
      result: Rc::new(RefCell::new(None)),
    })
  }

  fn callback_state(&self) -> (Rc<RefCell<Option<windows::core::Result<T>>>>, isize) {
    (self.result.clone(), self.event.0 as isize)
  }
...
#[cfg(target_arch = "aarch64")]
impl<T> Drop for Arm64ComCompletion<T> {
  fn drop(&mut self) {
...
#[cfg(target_arch = "aarch64")]
fn signal_arm64_completion<T>(
  result_cell: &Rc<RefCell<Option<windows::core::Result<T>>>>,
  event_handle: isize,
...
impl InnerWebView {
  ...
  fn create_environment(
    ...
    #[cfg(target_arch = "aarch64")]
    let completion = Arm64ComCompletion::new()?;
    #[cfg(target_arch = "aarch64")]
    let (result_cell, event_handle) = completion.callback_state();
    #[cfg(not(target_arch = "aarch64"))]
    ...
    unsafe {
      ...
      let handler = CreateCoreWebView2EnvironmentCompletedHandler::create(Box::new(
        move |error_code, environment| {
          ...
          signal_arm64_completion(&result_cell, event_handle, result)
        },
      ));
      #[cfg(not(target_arch = "aarch64"))]
      let handler = CreateCoreWebView2EnvironmentCompletedHandler::create(Box::new(
