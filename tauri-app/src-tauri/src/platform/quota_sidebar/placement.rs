...
pub(super) fn set_frame(window: &WebviewWindow, frame: SidebarFrame) -> Result<(), String> {
    ...
    if window.label() == super::LABEL {
        // WebKit paints on a different schedule from AppKit. Keep its viewport
        // fixed and clip with the true native window instead of resizing CSS.
        let left = native.screen().map(|screen| (frame.x - screen.visibleFrame().origin.x).abs() < 1.0).unwrap_or(false);
        unsafe { let _: () = objc2::msg_send![objc2::class!(CATransaction), begin];
        ...
        if let Some(content) = native.contentView() {
            ...
            if let Some(web) = sidebar_webview(&content) {
                web.setAutoresizingMask(objc2_app_kit::NSAutoresizingMaskOptions::empty());
                web.setFrame(NSRect::new(NSPoint::new(if left { 0.0 } else { frame.width - 88.0 }, (frame.height - 560.0) / 2.0), NSSize::new(88.0, 560.0)));
            }
            unsafe {
                let layer: *mut objc2::runtime::AnyObject = objc2::msg_send![&*content, layer];
                let color = objc2_app_kit::NSColor::colorWithSRGBRed_green_blue_alpha(0.031, 0.039, 0.035, 1.0);
                let cg: *const std::ffi::c_void = objc2::msg_send![&*color, CGColor];
                let _: () = objc2::msg_send![layer, setBackgroundColor: cg];
                let _: () = objc2::msg_send![layer, setMasksToBounds: true];
                let _: () = objc2::msg_send![layer, setCornerRadius: (8.0 + (frame.width - 16.0) * 14.0 / 72.0).clamp(8.0, 22.0)];
                let _: () = objc2::msg_send![layer, setMaskedCorners: if left { 10_usize } else { 5_usize }];
            }
        }
        unsafe { let _: () = objc2::msg_send![objc2::class!(CATransaction), commit]; }
...
#[cfg(target_os = "macos")]
fn sidebar_webview(view: &objc2_app_kit::NSView) -> Option<objc2::rc::Retained<objc2_app_kit::NSView>> {
    use objc2_foundation::NSObjectProtocol;
    for child in view.subviews().iter() {
        if child.isKindOfClass(objc2::class!(WKWebView)) { return Some(child); }
        if let Some(found) = sidebar_webview(&child) { return Some(found); }
    }
    None
}
