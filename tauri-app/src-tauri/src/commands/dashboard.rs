...
/// Small cache-only projection for the sidebar. Never schedules an aggregate or source scan.
#[tauri::command]
pub async fn read_sidebar_trend(
    window: tauri::WebviewWindow, app: AppHandle, source_token: CodexHomeSourceToken,
) -> Result<Vec<(i64, u64)>, String> {
    require_window_label(&window, "read_sidebar_trend")?;
    run_statistics_bound_dashboard_read(&app, source_token, move |codex_home| {
        let cached = token_count_jsonl::cached_last_good_dashboard_snapshot(&codex_home)?;
        Ok(cached.map(|snapshot| {
            let points = snapshot.recent_usage_24h;
            points.iter().skip(points.len().saturating_sub(288))
                .map(|point| (point.start_unix, point.tokens)).collect()
        }).unwrap_or_default())
    }).await
}
