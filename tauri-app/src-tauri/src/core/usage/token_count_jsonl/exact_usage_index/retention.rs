...
pub(super) fn validate(db: &Connection) -> Result<(), String> {
    ...
    if writer.as_deref().is_some_and(|v| v != "2" && v != "3" && v != "4" && v != WRITER_REVISION) {
...
fn install_transaction(db: &Connection, columns: &HashSet<String>, source_columns: &HashSet<String>, flavor: &str) -> Result<(), String> {
    ...
    // Null-to-known locations are additive, not displaced history.
    let changed = value_columns.iter().map(|c| format!("OLD.{c} IS NOT NEW.{c}"))
        .chain(location_columns.iter().filter(|c| columns.contains(**c))
            .map(|c| format!("(OLD.{c} IS NOT NULL AND OLD.{c} IS NOT NEW.{c})")))
        .collect::<Vec<_>>().join(" OR ");
...
mod tests {
    ...
    #[test]
    fn adding_missing_message_locations_does_not_duplicate_numeric_history() {
        let db=fixture();
        db.execute_batch("UPDATE event_rows SET user_prompt_start=50,user_prompt_end=60 WHERE ordinal=100;").unwrap();
        assert_eq!(scalar(&db,"SELECT COUNT(*) FROM retained_usage_revisions"),0);
        db.execute_batch("UPDATE event_rows SET user_prompt_start=NULL WHERE ordinal=100;").unwrap();
        assert_eq!(scalar(&db,"SELECT COUNT(*) FROM retained_usage_revisions"),1);
        assert_eq!(scalar(&db,"SELECT SUM(tokens) FROM event_rows"),100);
    }
