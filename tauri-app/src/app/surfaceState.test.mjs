...
test("windows updater lane uses signed Tauri metadata and a dashboard entry", async () => {
  ...
  const signScript = await readFile(new URL("../../../scripts/sign_tauri_windows_release.sh", import.meta.url), "utf8");
  ...
  assert.equal(buildScript.includes("build-manifest.json"), true);
  assert.equal(buildScript.includes(".sig"), false);
  assert.equal(signScript.includes("--key-path"), true);
  assert.equal(signScript.includes("latest-windows.json"), true);
