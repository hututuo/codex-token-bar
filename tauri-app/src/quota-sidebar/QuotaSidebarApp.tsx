...
import { sidebarTodayModels, sidebarLocalTime, sidebarMetricValue } from "./detailsModel";
...
function RailSurface() {
  ...
  const compactData = useMemo(() => sidebarData(data), [data.snapshot, data.runningThreads, data.quota.updatedAt, data.quota.account, data.quota.quota]);
  ...
  return <main className={sidebarRailClassName(display.quotaSidebarSide, state.mode)} onPointerDown={startDrag} onPointerEnter={enter} onPointerLeave={leave} data-dragging={dragging} aria-label="额度侧栏">
    <SidebarRailContent data={compactData} radar={radar} rateFullScale={rateFullScale} liveRateEnabled={display.liveRateEnabled} state={state} error={error} onOpen={tab => dispatch({ type: "open", tab })} />
  </main>;
}
export function SidebarRailContent({ data, radar, state, rateFullScale = 200, liveRateEnabled = true, error = null, onOpen }: {
  data: SidebarData; radar?: SidebarRadar; state: SidebarState; rateFullScale?: number; liveRateEnabled?: boolean; error?: string | null; onOpen: (tab: SidebarState["tab"]) => void;
}) {
  ...
  return <>
    ...
    {state.mode === "rest" ? <div className="qs-bars" aria-label="悬停查看额度">
      ...
      {showsFiveHour && <Bar percent={five} color="#b6ef75" />}<Bar percent={seven} color="#b4acff" />
    </div> : <div className="qs-summary">
      <span className="qs-caption">速览</span>
            <button className="qs-quota-trigger qs-rate-trigger" onClick={() => onOpen("quota")} aria-label="查看实时速率详情" title={rateLabel}><Ring label="t/s" percent={ratePercent} color="#78b7ff" /><Value value={ratePercent === null ? "—" : formatLiveRateValue(data.snapshot.tokensPerSecond)} /></button>
      {showsFiveHour && <button className="qs-quota-trigger" onClick={() => onOpen("quota")} aria-label="查看五小时额度详情"><Ring label="5h" percent={five} color="#b6ef75" /><Value value={quotaText(five)} /></button>}
      <button className="qs-quota-trigger" onClick={() => onOpen("quota")} aria-label="查看七天额度详情"><Ring label="7d" percent={seven} color="#b4acff" /><Value value={quotaText(seven)} /></button>
      <div className="qs-divider" />
      <button className="qs-running-trigger" onClick={() => onOpen("running")}><span className="qs-task-ring"><Value value={String(data.runningThreads.total ?? "—")} /></span><span>{data.runningThreads.status === "ready" ? `${data.runningThreads.mainThreads ?? "—"} 主 · ${data.runningThreads.subagents ?? "—"} 子` : data.runningThreads.status === "stale" ? "运行·过期" : "运行·未知"}</span></button>
      <button className="qs-recommendations" onClick={() => onOpen("radar")}><small>众测推荐{radar?.crowd.stale ? " · 旧" : ""}</small>{radar?.crowd.rows.slice(0, 3).map(row => <span key={row.rank} title={`${row.model} ${row.effort} · IQ ${row.iq.toFixed(1)}`}>{row.model.replace(/^gpt-[\d.]+-/, "")} {row.effort}</span>)}</button>
      <div className="qs-divider" />
      <button className="qs-window-status" data-active={sidebarRadarIsActive(radar)} onClick={() => onOpen("radar")}><span>{sidebarRadarCompact(radar)}</span>{sidebarRadarIsActive(radar) && radar?.official.deadlineMs && <small>{radar.official.countdown}</small>}</button>
...
function Bar({ percent, color }: { percent: number | null; color: string }) {
  return <span className={`qs-bar ${percent === null ? "qs-unknown" : ""}`}><i style={{ transform: `scaleY(${(percent ?? 100) / 100})`, background: color }} /></span>;
}
function Ring({ label, percent, color }: { label: string; percent: number | null; color: string }) {
  return <div className="qs-ring" data-known={percent !== null}><svg viewBox="0 0 52 52" aria-hidden="true"><circle cx="26" cy="26" r="22" className="qs-track" /><circle cx="26" cy="26" r="22" fill="none" stroke={color} strokeWidth="4" strokeLinecap="round" className="qs-ring-value" strokeDasharray="138.23" strokeDashoffset={138.23 * (1 - (percent ?? 0) / 100)} transform="rotate(-90 26 26)" /></svg><span>{label}</span></div>;
}
function DetailSurface() {
  ...
  const action = (action: SidebarAction) => void emitTo(railLabel, "quota-sidebar-action", action);
...
export function QuotaDetails({ data, onOpenRunning }: { data: SidebarData; onOpenRunning?: () => void }) {
  ...
  const models = sidebarTodayModels(snapshot.todayModelBreakdowns);
  const sevenDayReference = !snapshot.quotaDataStale
    ? quotaPercent(snapshot.sevenDayAvailability, snapshot.sevenDayExpectedRemainingPercent ?? null) : null;
  ...
  return <>
    ...
    <div className="qs-quota-group">
      {([ ["5 小时额度", quota.quota.fiveHour, "#b6ef75"], ["7 天额度", quota.quota.sevenDay, "#b4acff"] ] as const)
        ...
        .map(([label, limit, color]) => {
          ...
          return <section className="qs-limit" key={label}>
            ...
            {label === "7 天额度" && sevenDayReference !== null && <p className="qs-pace-reference">按周期均匀使用，此刻参考剩余 {quotaText(sevenDayReference)}</p>}
          </section>;
        })}
    </div>
    <section className="qs-usage"><div className="qs-section-heading"><h3>Token 用量</h3><span>今日 / 累计</span></div>
    ...
    <section className="qs-model-usage"><div className="qs-section-heading"><h3>今日模型用量</h3><span>今日占比 · 全部模型为分母</span></div>
      {models.length === 0 ? <p className="qs-empty">暂无可信的今日模型用量</p> : models.map((model, index) => <div className="qs-model-row" key={model.model ?? "__unknown__"}>
        <div><strong title={model.model ?? "模型未知"}>{model.model || "模型未知"}</strong><span>{formatTokens(model.tokens)} <small>{model.percent.toFixed(1)}%</small></span></div>
        <progress max={100} value={model.percent} style={{ "--qs-accent": ["#b6ef75", "#b4acff", "#efc077", "#80c6bc"][index] } as CSSProperties} />
      </div>)}
    </section>
...
export function RadarDetails({ radar }: { radar: SidebarRadar }) {
  ...
  return <>
    ...
    <section className="qs-radar-ranking"><div className="qs-section-heading"><h3>众测实时排行</h3><span>{!crowd.available ? "未知" : crowd.stale === true ? "已过期" : crowd.stale === false ? "来源标记新鲜" : "新鲜度未知"}</span></div>
      <p className="qs-radar-basis">每格最新一次 · 至少 {crowd.minimumSamples} 个样本 · 沿用雷达排序</p>
