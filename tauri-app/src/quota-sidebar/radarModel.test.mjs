...
test("radar details show true numbered rows and distinct window/action/IQ/ranking sections", async () => {
  await withSsrModules(async load => {
    ...
    for (const value of ["速登状态", "速登窗口", "建议动作", "测评 IQ 摘要", "雷达站模型排行", "众测雷达", "gpt-6-astra", "141.0"]) assert.ok(html.includes(value), value);
    assert.equal((html.match(/class="qs-rank-number"/g) || []).length, 1);
