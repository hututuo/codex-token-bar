import {
  radarIsSpeedWindow, radarSpeedWindowDeadlineMs, primaryModelMeasurementRow, radarActionDisplayTextForSnapshot, radarWindowSummaryDisplayText,
...
export interface SidebarRadar {
  ...
  crowd: {
    available: boolean; stale: boolean | null; updatedAt: string; source: string; minimumSamples: number;
...
export function sidebarRadarSnapshot(official: CodexRadarSnapshot | null, crowd: CodexCrowdRadarSnapshot | null, nowMs = Date.now()): SidebarRadar {
  ...
  return {
    ...
    crowd: { available: crowd !== null && crowd.realtimeAvailable, stale,
