...
struct QuotaSidebarRadarContent: View {
    ...
    var initialShowsCrowd = false
    @State private var showsCrowd = false
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 7) {
                Text(officialStale ? "建议暂未刷新" : "建议动作")
            ...
            Picker("排名来源", selection: $showsCrowd) {
                Text("雷达站").tag(false)
                Text("众测雷达").tag(true)
            }.pickerStyle(.segmented).id("ranking")
            if !showsCrowd {
                let rows = snapshot.map { ([$0.modelIQ.primaryModelRow] + $0.modelIQ.secondaryModelRows).filter { $0.point.hasMeasurement } } ?? []
                Text("雷达站模型排行 · 按测评 IQ 排序").font(.system(size: 11)).foregroundStyle(.gray)
                if rows.isEmpty { Text("暂无测评排名").font(.system(size: 11)).foregroundStyle(.gray) }
                ForEach(Array(rows.enumerated()), id: \.offset) { index, row in
                    HStack(spacing: 10) {
                        Text("\(index + 1)").foregroundStyle(.cyan).frame(width: 20)
                        VStack(alignment: .leading, spacing: 5) {
                            Text(row.point.modelDisplayName).font(.system(size: 11))
                            Text("\(row.point.passed)/\(row.point.tasks) 通过").font(.system(size: 9)).foregroundStyle(.gray)
                        }
                        Spacer()
                        Text(CodexRadarModelIQPoint.display(row.point.score) + " IQ").monospacedDigit()
            ...
            .onAppear { showsCrowd = initialShowsCrowd }
    }

    private var staleBadge: some View {
        Text("刷新未成功").font(.system(size: 9)).foregroundStyle(.orange)
