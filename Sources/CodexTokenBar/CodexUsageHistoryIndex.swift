...
private struct CodexUsageSourceChangedError: LocalizedError {
    let path: String
...
final class CodexUsageHistoryIndex: @unchecked Sendable {
        ...
        try driver.withConnection { connection in
            ...
            for file in files {
                try autoreleasepool {
                    ...
                    if let existing = try indexedSource(path: path, connection: connection),
                    ...
                    let result = try rebuildSource(
                        file: canonicalFile,
                        sessionID: sessionID(canonicalFile),
                    ...
                    changedFiles += 1
                    indexedEvents += result.eventCount
                }
            }
    ...
    private func prepareSchema() throws {
        try driver.withConnection { connection in
            ...
            if currentVersion != nil, currentVersion != Self.schemaVersion {
                try connection.execute(
                    """
    ...
    private func rebuildSource(
        ...
        try connection.transaction { transaction in
            ...
            let result = try parser(
                file,
                sessionID,
                observedSignature.size,
                { fingerprint in
                    try fingerprintStatement.execute([.text(fingerprint.databaseKey)]) > 0
                },
                { indexedEvent in
