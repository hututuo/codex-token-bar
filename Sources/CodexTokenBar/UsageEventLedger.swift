...
enum UsageEventLedger {
    ...
    static func admit(_ row: Candidate, source: Int64, generation: String, admission: Admission,
        ...
        if let existing {
            ...
            if same || verifiedLocation {
                ...
                try enrichTurn(source: source, event: existing, generation: generation,
                               prompt: row.promptOffset, assistant: row.assistantOffset, on: db)
    ...
    // Metadata enrichment cannot replace a previously proved logical turn or
    // change any numeric fact. Raw offsets belong to the binding generation.
    static func enrichTurn(source: Int64, event: Int64, generation: String,
                           prompt: Int64?, assistant: Int64?, on db: SQLiteDatabaseConnection) throws {
        guard let prompt else { return }
        let existing = try db.readRows("SELECT user_prompt_offset FROM events WHERE source_id=? AND source_offset=?", bindings:[.int64(source),.int64(event)]) { $0.int64(0) }
        guard !existing.isEmpty else { return }
        var turn = existing[0]
        if turn == nil {
            turn = try db.readRows("SELECT turn_id FROM usage_ledger_turns WHERE source_id=? AND generation=? AND raw_offset=?", bindings:[.int64(source),.text(generation),.int64(prompt)]) { $0.int64(0) }.first ?? nil
            if turn == nil { turn = try allocate(on: db) }
            try db.execute("UPDATE events SET user_prompt_offset=?,assistant_start_offset=COALESCE(assistant_start_offset,?) WHERE source_id=? AND source_offset=? AND user_prompt_offset IS NULL", bindings:[optional(turn),optional(assistant),.int64(source),.int64(event)])
        }
        try db.execute("INSERT OR IGNORE INTO usage_ledger_turns VALUES (?,?,?,?)", bindings:[.int64(source),.text(generation),.int64(prompt),optional(turn)])
    }
