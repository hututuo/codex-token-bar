...
struct DashboardQuotaCycleBrowser: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .buttonStyle(.plain).font(.system(size: 11, weight: .medium)).padding(.horizontal, 12)
            if calendarExpanded {
                QuotaCycleCalendar(cycles: cycles, selectedID: selected?.id) { selectedID = $0 }
            }

            if let selected {
                Text("\(selected.start.map { boundaryText($0) } ?? "从 \(dateText(selected.firstObservedAt)) 开始记录") → \(selected.end.map { boundaryText($0) } ?? "至今")")
                    .font(.system(size: 10)).foregroundStyle(.secondary).padding(.horizontal, 12)
                if selected.isCurrent {
                    Text("计划重置 \(dateText(selected.scheduledResetAt))")
                        .font(.system(size: 10)).foregroundStyle(.secondary).padding(.horizontal, 12)
                }
            } else {
                Text("暂无周期记录。")
            ...
            if let value = visibleResult {
                Text("已观测部分 \(value.total.totalTokens.abbreviatedTokens) Token · 输入 \(value.total.inputTokens.abbreviatedTokens) · 缓存 \(value.total.cachedInputTokens.abbreviatedTokens) · 输出 \(value.total.outputTokens.abbreviatedTokens) · \(value.total.calls) 次")
                    .font(.system(size: 10)).foregroundStyle(.secondary).padding(.horizontal, 12)
            }
            if let errorText {
                Text(errorText).font(.system(size: 10)).foregroundStyle(.secondary).padding(.horizontal, 12)
            }
