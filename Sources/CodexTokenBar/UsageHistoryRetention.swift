...
enum UsageHistoryRetention {
    ...
    static func validate(on db: SQLiteDatabaseConnection) throws {
        ...
        guard writer == nil || writer == "2" || writer == "3" || writer == "4" || writer == writerRevision else {
    ...
    static func install(on db: SQLiteDatabaseConnection) throws {
        ...
        try db.transaction { tx in
            ...
            let valueColumns = ["source_id", "source_offset", "timestamp", "tokens", "input_tokens", "cached_input_tokens", "output_tokens", "reasoning_output_tokens", "model"]
                + ["accounting_kind", "reported_total_tokens", "legacy_tokens"].filter { columns.contains($0) }
            // Filling a missing location adds information; only replacing a
            // known location needs a retained copy of the old observation.
            let changed = (valueColumns.map { "OLD.\($0) IS NOT NEW.\($0)" }
                + ["user_prompt_offset", "assistant_start_offset"].map {
                    "(OLD.\($0) IS NOT NULL AND OLD.\($0) IS NOT NEW.\($0))"
                }).joined(separator: " OR ")
