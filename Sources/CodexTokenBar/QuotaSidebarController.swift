...
final class QuotaSidebarController: NSObject, ObservableObject {
    ...
    func show(store: CodexUsageStore, monitor: LiveRateMonitor, quota: AccountQuotaStore,
              tasks: TaskCompletionMonitor, radar: CodexRadarStore, edge: QuotaSidebarEdge) {
        ...
        if railPanel == nil {
            ...
            makeRail = { [weak self] in
                ...
                return AnyView(QuotaSidebarRail(controller: self, quota: quota, tasks: tasks, radar: radar, monitor: monitor))
            }
            makeDetail = { [weak self] in
                guard let self else { return AnyView(EmptyView()) }
                return AnyView(QuotaSidebarDetailView(controller: self, store: store, monitor: monitor, quota: quota, tasks: tasks, radar: radar))
            }
    ...
    func select(_ detail: QuotaSidebarDetail) {
        guard !isTrackingPress, !isDragging else { return }
        collapseTask?.cancel(); collapseTask = nil
