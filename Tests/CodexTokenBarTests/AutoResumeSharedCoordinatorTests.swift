...
final class AutoResumeSharedCoordinatorTests: XCTestCase {
    ...
    func testLedgerFileLockRejectsConcurrentWriterAndPreservesBothClaims() throws {
        let home = try temporaryCodexHome()
        defer { try? FileManager.default.removeItem(at: home) }
        let first = AutoResumeSharedCoordinator(codexHome: home, ownerID: "swift")
        let second = AutoResumeSharedCoordinator(codexHome: home, ownerID: "tauri")
        let firstKey = "daily:thread-lock:2026-07-16:0900"
        let secondKey = "daily:thread-lock:2026-07-16:1000"

        XCTAssertTrue(try first.claimTrigger(
            key: firstKey,
            threadID: "thread-lock",
            minimumInterval: 0,
            now: Date(timeIntervalSince1970: 30_000)
        ))
        try first.withLedgerLock {
            XCTAssertThrowsError(try second.withLedgerLock(waitTimeout: 0) {}) { error in
                XCTAssertEqual(
                    error as? AutoResumeCoordinationError,
                    .ledgerLockTimedOut
                )
            }
        }
        XCTAssertTrue(try second.claimTrigger(
            key: secondKey,
            threadID: "thread-lock",
            minimumInterval: 0,
        ...
        let ledger = try JSONDecoder().decode(
            ...
            from: Data(
                contentsOf: first.rootDirectory
                    .appendingPathComponent("trigger-ledger.json")
            )
        )
        XCTAssertEqual(Set(ledger.entries.keys), Set([firstKey, secondKey]))
    }
