...
final class UsageHistoryRetentionTests: XCTestCase {
    ...
    func testAddingMissingMessageLocationsDoesNotDuplicateNumericHistory() throws {
        let db = try fixture()
        try db.execute("UPDATE events SET user_prompt_offset=50,assistant_start_offset=60 WHERE source_offset=100;")
        XCTAssertEqual(try scalar("SELECT COUNT(*) FROM retained_usage_revisions",db),0)
        try db.execute("UPDATE events SET user_prompt_offset=NULL WHERE source_offset=100;")
        XCTAssertEqual(try scalar("SELECT COUNT(*) FROM retained_usage_revisions",db),1)
